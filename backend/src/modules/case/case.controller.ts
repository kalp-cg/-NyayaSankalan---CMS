 ...
 * AI copilot brief for a case (with local fallback)
 */
export const getCaseCopilotBrief = asyncHandler(async (req: Request, res: Response) => {
  const { caseId } = req.params;
  const userRole = req.user!.role;
  const organizationId = req.user!.organizationId;

  const result = await caseService.getCaseCopilotBrief(caseId, userRole, organizationId);
