...
const ChatbotWidget: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  const [messages, setMessages] = useState<Message[]>([
    { sender: 'bot', text: 'Hello! I am your Legal Co-Pilot. Ask about FIR sections, arrest status, IPC/BNS section meaning, evidence gaps, or a specific FIR number.' }
  ]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const message = e?.response?.data?.error || e?.response?.data?.message || 'Sorry, there was an error contacting the AI service.';
      setMessages((msgs) => [...msgs, { sender: 'bot', text: message }]);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed bottom-20 right-6 z-50 w-80 max-w-full bg-white rounded-lg shadow-2xl border border-gray-200 flex flex-col" style={{ height: 420 }}>
      ...
      <div className="p-2 border-t bg-white flex gap-2">
        <input
          ...
          placeholder="Ask e.g. Under which section was FIR 001/2026 registered?"
          value={input}
