...
import type { Case, AssignCaseFormData, ApiResponse, CaseCopilotBrief } from '../types/api.types';
...
export const caseApi = {
  ...
  /**
   * GET /api/cases/:caseId/copilot-brief
   * Fetch AI copilot brief for a specific case
   */
  getCopilotBrief: async (caseId: string): Promise<CaseCopilotBrief> => {
    const response = await apiClient.get<ApiResponse<CaseCopilotBrief>>(`/cases/${caseId}/copilot-brief`);
    return response.data.data || {
      summary: 'AI brief unavailable.',
      keyRisks: [],
      nextActions: [],
      missingItems: [],
      confidence: 0,
